...
SECTION_TYPE = typing.Literal[
    "advanced",
    "simple",
]


def get_forecast_navigation_sections(
    session: sqlmodel.Session, *, type_: SECTION_TYPE
) -> list[schemas.ForecastCoverageNavigationSection]:
    sections = {}
    inclusion_attribute = {
        "advanced": "include_in_advanced_section_combinations",
        "simple": "include_in_simple_section_combinations",
    }[type_]
    for cov_conf in collect_all_forecast_coverage_configurations(session):
        if getattr(cov_conf, inclusion_attribute, False):
...
def get_historical_navigation_sections(
    session: sqlmodel.Session, *, type_: SECTION_TYPE
