...
class OverviewCoverageConfigurationView(ModelView):
    identity = "overview_coverage_configurations"
    name = "Overview Coverage Configuration"
    label = "Overview Coverage Configurations"
    pk_attr = "id"
    ...
    fields = (
        ...
        starlette_admin.EnumField(
            "data_category", enum=static.DataCategory, required=True
        ),
        fields.RelatedClimaticIndicatorField(
            "climatic_indicator",
            help_text="climatic indicator",
    ...
    @staticmethod
    def _serialize_instance(instance: coverages.OverviewCoverageConfiguration):
        return read_schemas.OverviewCoverageConfigurationRead(
            **instance.model_dump(
                exclude={
                    "scenarios",
                    "climatic_indicator",
    ...
    async def create(self, request: Request, data: dict[str, Any]) -> Any:
        try:
            data = await self._arrange_data(request, data)
            await self.validate(request, data)
            overview_coverage_configuration_create = (
                coverages.OverviewCoverageConfigurationCreate(
                    netcdf_main_dataset_name=data["netcdf_main_dataset_name"],
                    thredds_url_pattern=data["thredds_url_pattern"],
                    climatic_indicator_id=data["climatic_indicator"],
            ...
            db_overview_coverage_configuration = await anyio.to_thread.run_sync(
                database.create_overview_coverage_configuration,
                request.state.session,
                overview_coverage_configuration_create,
            )
            return self._serialize_instance(db_overview_coverage_configuration)
        except Exception as e:
            return self.handle_exception(e)
    ...
    async def edit(self, request: Request, pk: Any, data: dict[str, Any]) -> Any:
        try:
            data = await self._arrange_data(request, data, True)
            await self.validate(request, data)
            overview_coverage_configuration_update = (
                coverages.OverviewCoverageConfigurationUpdate(
                    netcdf_main_dataset_name=data["netcdf_main_dataset_name"],
                    thredds_url_pattern=data["thredds_url_pattern"],
                    climatic_indicator_id=data["climatic_indicator"],
                    data_category=data["data_category"],
            ...
            db_overview_coverage_configuration = await anyio.to_thread.run_sync(
                database.get_overview_coverage_configuration, request.state.session, pk
            )
            db_overview_coverage_configuration = await anyio.to_thread.run_sync(
                database.update_overview_coverage_configuration,
                request.state.session,
    ...
    async def find_by_pk(
        self, request: Request, pk: Any
    ) -> read_schemas.OverviewCoverageConfigurationRead:
        db_item = await anyio.to_thread.run_sync(
            database.get_overview_coverage_configuration, request.state.session, pk
        )
        return self._serialize_instance(db_item)
    ...
    async def find_all(
        ...
        where: Union[dict[str, Any], str, None] = None,
        order_by: Optional[list[str]] = None,
    ) -> Sequence[read_schemas.OverviewCoverageConfigurationRead]:
        finder = functools.partial(
            database.collect_all_overview_coverage_configurations,
        )
        db_items = await anyio.to_thread.run_sync(finder, request.state.session)
        return [self._serialize_instance(ind) for ind in db_items]
