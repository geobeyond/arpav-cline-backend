...
class MonthlyMeasurementView(ModelView):
    ...
    async def find_all(
        ...
        list_measurements = functools.partial(
            database.list_monthly_measurements,
            limit=limit,
            offset=skip,
            include_total=False,
        )
...
class SeasonalMeasurementView(ModelView):
    ...
    async def find_all(
        ...
        list_measurements = functools.partial(
            database.list_seasonal_measurements,
            limit=limit,
            offset=skip,
            include_total=False,
        )
...
class YearlyMeasurementView(ModelView):
    ...
    async def find_all(
        ...
        list_measurements = functools.partial(
            database.list_yearly_measurements,
            limit=limit,
            offset=skip,
            include_total=False,
        )
...
class StationView(ModelView):
    ...
    async def edit(
        ...
        try:
            ...
            db_station = await anyio.to_thread.run_sync(
                database.get_station, request.state.session, pk
            )
            db_station = await anyio.to_thread.run_sync(
                database.update_station,
                request.state.session,
                db_station,
                station_update,
            )
...
class ObservationSeriesConfigurationView(ModelView):
    ...
    label = "Observation Series Configurations"
    icon = "fa fa-blog"
    pk_attr = "id"
    fields = (
        fields.UuidField("id"),
